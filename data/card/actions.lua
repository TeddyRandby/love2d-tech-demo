...
---@class ActionInputMicroOpOpponentPeek
---@field type "opponent_peek"
---@field amount integer
...
---@alias ActionInputMicroOp ActionInputMicroOpOpponentPeek | ActionInputMicroOpPull | ActionInputMicroOpPeek | ActionInputMicroOpConstant | ActionInputMicroOpSignature
...
---@param n integer
function M.input.opponent_peek(n)
	---@type ActionInputMicroOp
	return {
		type = "opponent_peek",
		amount = n,
...
function M.refine(n)
	...
	return {
		name = "refine",
		desc = "Look at " .. n .. " tokens from your bag. Draft any.",
		microops = {
			M.input.peek(n),
			M.intermediate.choose(1),
			M.output.discard(),
...
function M.steal(n)
	...
	return {
		name = "steal",
		desc = "Look at " .. n .. " tokens from your opponents bag. Draft any, and donate the rest.",
		microops = {
			M.input.opponent_peek(n),
			M.intermediate.choose(1),
			M.output.donate(),
			M.output.draft(),
		},
...
---@param n integer
function M.opponent_refine(n)
	---@type ActionOp
	return {
		name = "opponent_refine",
		desc = "Look at " .. n .. " tokens from your opponents bag. Donate any.",
		microops = {
			M.input.opponent_peek(n),
			M.intermediate.choose(1),
			M.output.discard(),
