...
---@alias RenderCommand RenderCommandIcon | RenderCommandText | RenderCommandCard | RenderCommandToken | RenderCommandBag | RenderCommandMove | RenderCommandBoardSlot | RenderCommandMeta | RenderCommandDetails
...
function M:icon(icons, x, y, id, r, ox, oy, time)
  self:push_renderable("icon", icons, id or {}, move_contains, x, y, r, ox, oy, time)
end
...
function M:move(move, x, y, id)
  ...
  if move then
    self:push_renderable("icon", move.icon, {}, nil, x, y)
  end
end
...
function M:__drawcommand(v)
  ...
  elseif t == "button" then
    ...
    local target = v.target
    local pos = self.command_target_positions[v.id]
    UI.button.draw(target, pos.x, pos.y, target.text)
